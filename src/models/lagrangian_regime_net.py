...
class LagrangianConfig:
    ...
    # Skip connection: injects last-row features directly into latent state
...
class LagrangianRegimeNet(nn.Module):
    ...
    def __init__(self, cfg: LagrangianConfig) -> None:
        ...
        if cfg.use_skip_connection:
            self.skip_proj = nn.Linear(cfg.input_dim, cfg.latent_dim)
        ...
        if cfg.use_transition_head:
            ...
            nn.init.zeros_(self.transition_head.bias)
    ...
    def forward_with_transition(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Returns (regime_logits (B,4), transition_logits (B,1)).
        Requires use_transition_head=True in config.
        """
        if not self.cfg.use_transition_head:
            raise RuntimeError("forward_with_transition requires use_transition_head=True")
        h = self._encode(x)
        z = self.z0_head(h)
        ...
        z_norm = self.norm(z)
        return self.head(z_norm), self.transition_head(z_norm)
