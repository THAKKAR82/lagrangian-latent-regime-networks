...
|---|---|---|
| Match or exceed GRU | Mean macro F1 >= 0.401 | **Achieved** — Ensemble: 0.4046 |
| Approach or exceed LSTM | Mean macro F1 >= 0.406 | Not yet — Ensemble: 0.4046 vs LSTM 0.4062 |
| Remain competitive on calibration | Mean ECE near 0.147, Brier near LSTM/GRU levels | **Achieved** — Ensemble ECE 0.121, Brier 0.590 (better than all baselines) |
| Maintain reproducibility | 71-fold rerun with consistent aggregate metrics | Achieved — saved as `walk_forward_summary_ensemble.csv` |
...
- Can the ensemble be improved by weighting XGBoost higher in early folds (where it collapses) and conv1d higher in late folds?
- Does temperature scaling on the ensemble probabilities improve ECE further toward 0.10?[20][27]

**Answered:**
- Econophysics features with conv1d (single-horizon): No — consistently -2pp vs baseline across all encoder variants tested.
- Why does XGBoost lead: realized vol (21/63d) + cross-asset correlations (63d) + VIX level account for 59% of XGBoost's feature importance. XGBoost operates on pre-aggregated scalars and creates non-linear interactions efficiently. It fails in early folds (2007-2008) due to insufficient training data.
- Which periods favor structured dynamics: conv1d wins in early low-data folds (Q1) and some late folds; XGBoost dominates Q2 (2009-2013 recovery). Fold F1 Pearson r = 0.29 — complementary signals, which motivated the ensemble.
